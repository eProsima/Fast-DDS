...
It will contain 15 samples, 5 per key.
...
Audio and Video transmission have a common characteristic: Having a stable, high data-rate feed is more important than having a 100% lossless transmission.
...
makes decisions based on the temperature distribution. We would group all sensors within one single topic,
    ...
    Keys: We use a separate key for each sensor. That way new messages from one sensor don't affect the storage of past samples from another sensor.
    Durability: Transient Local. We don't want to lose samples due a sensor losing connection temporarily.
...
In the first one launch: './DDSConfigurabilityPublisher' (or DDSConfigurabilityPublisher.exe on windows).
In the second one: './DDSConfigurabilitySubscriber' (or DDSConfigurabilitySubscriber.exe on windows).
