	...
	while(std::cin >> ch)
	{
		if(ch == 'f')
		{
         st.wasFast(true);
			...
			cout << "Sending sample, count=" << msgsent_fast << " through the fast writer. Send another sample? (f-fast,s-slow,q-quit): ";
		}
		else if(ch == 's')
		{
         st.wasFast(false);
			mp_slow_publisher->write(&st);  ++msgsent_slow;
			cout << "Sending sample, count=" << msgsent_slow << " through the slow writer. Send another sample? (f-fast,s-slow,q-quit): " << std::endl;
		}
      else if (ch == 'q')
      {
			cout << "Finishing Flow Control example" << std::endl;
			break;
      }
		else
		{
			cout << "Command " << ch << " not recognized, please enter \"f/s/q\":";
		}
