...
/**
 * @file BigDataTransportPublisher.cpp
 *
 */
...
#include "BigDataTransportPublisher.h"
...
void HelloWorldPublisher::fill_data(
        uint32_t length)
{
    uint8_t num = static_cast<uint8_t>(421);
    auto fillnum = [&]()
            {
                return num;
            };
    ...
    std::generate(data.begin(), data.end(), fillnum);

    bigdata_.data(data);
}

void HelloWorldPublisher::set_timestamp(
        BigData& big_data)
{
...
size_t HelloWorldPublisher::get_size()
{
...
{
    fill_data();
    // size_t size = eprosima::fastcdr::calculate_serialized_size(bigdata_);
    std::cout << "Msg size in bytes: " << get_size() << std::endl;
    bigdata_.index(0);
    ...
    else
    {
        ...
        std::cout << "Creating participant from XML profile" << std::endl;
...
void HelloWorldPublisher::runThread(
        ...
        uint32_t sleep)
{
    while (!is_stopped() && (samples == 0 || bigdata_.index() < samples))
    {
        if (listener_.enough_matched())
        {
            auto time_now = std::chrono::high_resolution_clock::now();

            auto time_elapsed = time_now - time_start; // In nanoseconds

            publish();

            double throughput = double(bytes_sent) / (time_elapsed.count() / double(1000));

            std::cout << "[" << bigdata_.index() << "]"
                      << " TP (MB/s): " <<  throughput << std::endl;
...
{
    bigdata_.index()++;
    set_timestamp(bigdata_);
    bytes_sent += get_size();
    writer_->write(&bigdata_);
}
