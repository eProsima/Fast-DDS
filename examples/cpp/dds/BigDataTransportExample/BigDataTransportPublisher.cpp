        ...
        const std::string& profile)
{
    ...
    bigdata_.index(0);
    // std::cout << "Msg size in bytes: " << bigdata_.getCdrSerializedSize(msg_) << std::endl;
    ...
    if (profile.empty())
    {
        std::cout << "Please load XML file" << std::endl;
        return false;
    }
    else
    {
        ...
        uint32_t sleep)
{
    while (!is_stopped() && (samples == 0 || bigdata_.index() < samples))
    {
        ...
        {
            publish();
            std::cout << "Message with index: " << bigdata_.index()
                      << " SENT" << std::endl;
...
{
    bigdata_.index()++;
    dds_types_bigdata::set_bigdata_timestamp_to_now(bigdata_);
