...
/**
 * @file BigDataTransport_main.cpp
 *
 */
        ...
        char** argv)
{
    ...
    std::string topic_name = "BigDataTopic";
    int count = 0;
    long sleep = 100;
    int num_wait_matched = 0;
    int domain = 0;
    bool async = false;
    BuiltinTransports btransports = BuiltinTransports::DEFAULT;
    bool reliable = false;
    ...
    for (int i = 0; i < parse.optionsCount(); ++i)
    {
        ...
        switch (opt.index())
        {
            ...
            case optionIndex::TRANSPORT:
                if (strcmp(opt.arg, "default") == 0)
                {
                    btransports = BuiltinTransports::DEFAULT;
                }
                else if (strcmp(opt.arg, "udp") == 0 || (strcmp(opt.arg, "udpv4") == 0))
                {
                    btransports = BuiltinTransports::UDPv4;
                }
                else if (strcmp(opt.arg, "shm") == 0)
                {
                    btransports = BuiltinTransports::SHM;
                }
                else if (strcmp(opt.arg, "large_data") == 0)
                {
                    btransports = BuiltinTransports::LARGE_DATA;
                }
                break;
    ...
    switch (type)
    {
        case PUBLISHER:
        {
            ...
            if (mypub.init(topic_name, static_cast<uint32_t>(domain), static_cast<uint32_t>(num_wait_matched), async,
                    btransports, reliable, transient, partitions, use_ownership, ownership_strength,
        ...
        case SUBSCRIBER:
        {
            ...
            if (mysub.init(topic_name, static_cast<uint32_t>(count), static_cast<uint32_t>(domain), btransports,
                    reliable, transient, partitions, use_ownership, participant_profile,
                    static_cast<uint32_t>(history)))
            {
