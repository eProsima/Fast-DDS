...
void aux_array_print(nlohmann::json& j_array, const std::vector<unsigned int>& dims, unsigned int& index)
{
    ...
    for (unsigned int i = 0; i < dims[0]; ++i)
    {
        if (dims.size() == 1)
        {
            // print_basic_element_json(inner_array); // print_element_json(inner_array)
            inner_array.push_back(index);
            ++index;
        }
        else
        {
            aux_array_print(inner_array, std::vector<unsigned int>(dims.begin() + 1, dims.end()), index);
        }
    }
    ...
    if (nullptr != data)
    {
        switch (data->type()->get_kind())
        {
            case TK_STRUCTURE:
            {
                ...
                // std::array<unsigned int, 2> inner_array1 = {1, 2};
                // std::array<unsigned int, 2> inner_array2 = {3, 4};
                // j_array.push_back(inner_array1);
                // j_array.push_back(inner_array2);
                // // std::cout << std::setw(2) << j_array << std::endl;

                // nlohmann::json j_array_2 = nlohmann::json::array({{1, 2}, {3, 4}});
                // // std::cout << std::setw(2) << j_array_2 << std::endl;
                // // std::cout << j_array_2[1][1] << std::endl;

                // nlohmann::json j_array_3 = nlohmann::json::array();
                // j_array_3[0][0] = 1;
                // j_array_3[0][1] = 2;
                // j_array_3[1][0] = 3;
                // j_array_3[1][1] = 4;
                // std::cout << j_array_3 << std::endl;

                std::vector<unsigned int> dims = {3,3,3};
                unsigned int total_size = 27;
                unsigned int idx = 0;
                nlohmann::json j_array = nlohmann::json::array();
                aux_array_print(j_array, dims, idx);
                ...
                // j_array[1][0] = 3;
                // j_array[1][1] = 4;
                // j_array[0] = nlohmann::json::array();
                // j_array[0][1][0] = nlohmann::json::array();
                // j_array[0][1][0].push_back(1);
                // j_array[0][0] = 3;
...
void DynamicDataHelper::print_json_stream(
        const DynamicData::_ref_type& data,
        std::ostream& output)
{
    if (nullptr != data)
        ...
        const std::string& tabs)
{
    ...
    switch (desc->type()->get_kind())
    {
        ...
        case TK_UNION:
        {
            ...
            print_member(st_data, selected_member_type, tabs + "\t");
            data->return_loaned_value(st_data);
        ...
        case TK_ALIAS:  // TODO: move to helper that gets actual kind from alias (like resolve_alias_enclosed_type)
        {
            MemberDescriptor::_ref_type alias_member_desc {traits<MemberDescriptor>::make_shared()};
            type->get_descriptor(alias_member_desc); // TODO: already done above??
            traits<DynamicType>::ref_type alias_type = alias_member_desc->type();

            TypeDescriptor::_ref_type alias_desc {traits<TypeDescriptor>::make_shared()};
            do {
                alias_type->get_descriptor(alias_desc);
                alias_type = alias_desc->base_type();
            } while (TK_ALIAS == alias_type->get_kind());
            // print_member(data, alias_type, tabs);
            print_member_alias_aux(data, type->get_id(), alias_type->get_kind(), tabs, alias_type);
            break;
        }
        default:
            break;
...
void DynamicDataHelper::print_member_alias_aux(
        DynamicData::_ref_type data,
        const MemberId& id,
        const TypeKind& kind,
        const std::string& tabs,
        DynamicType::_ref_type enclosing_type)
{
    switch (kind)
    {
        case TK_NONE:
        ...
        case TK_BITMASK:
        {
            print_basic_element(data, id, kind);
            std::cout << std::endl;
            break;
        }
        case TK_STRUCTURE:
        case TK_BITSET:
        {
            ...
            DynamicTypeMembersById members;
            // st_data->type()->get_all_members(members);
            enclosing_type->get_all_members(members);
            // traits<DynamicData>::narrow<DynamicDataImpl>(st_data)->get_enclosing_type()->get_all_members(members); // TODO: once in fast
            for (auto it : members)
            {
                print_member(st_data, it.second, tabs + "\t");
            }
        ...
        case TK_MAP:
        {
            ...
            {
                std::cout << tabs << "\tKey: ";
                auto map_id = st_data->get_member_id_at_index(i);
                std::cout << std::to_string(map_id);
                std::cout << tabs << "\tValue: ";
                print_element(st_data, map_id, value_type, tabs + "\t");
                std::cout << std::endl;
            }
            data->return_loaned_value(st_data);
            break;
        ...
        TypeKind kind)
{
    switch (kind)
    {
        ...
        case TK_BITMASK:
        {
            ...
            data->get_descriptor(bitmask_member_desc, id);

            TypeDescriptor::_ref_type bitmask_desc {traits<TypeDescriptor>::make_shared()};
            bitmask_member_desc->type()->get_descriptor(bitmask_desc);

            auto bound = bitmask_desc->bound().at(0);

            // DynamicTypeMembersById bitmask_members;
            // bitmask_member_desc->type()->get_all_members(bitmask_members);
            // for (auto it : bitmask_members)
            // {
            //     bool value;
            //     data->get_boolean_value(value, it.second->get_id());
            //     std::cout << it.second->get_name() << ": " << (value ? "true" : "false") << std::endl;
            // }
        ...
        nlohmann::json& j)
{
    switch (kind)
    {
        ...
        case TK_ENUM: // TODO: print descriptor name instead of raw value
        {
            ...
            j[member_name] = { {"name", name}, {"literal", value} };
            break;
        }
        case TK_INT64:
        {
        ...
        nlohmann::json& j)
{
    ...
    switch (descriptor->element_type()->get_kind())
    {
        ...
        case TK_BITMASK:
        {
            print_basic_collection_json(data, j);
            break;
        }
        case TK_STRUCTURE:
        case TK_BITSET:
        ...
        case TK_MAP:
        {
            print_complex_collection_json(data, j);
            break;
        }
        default:
            break;
        ...
        nlohmann::json& j)
{
    if (data->type()->get_kind() == TK_SEQUENCE)
    {
        ...
        for (uint32_t i = 0; i < count; ++i)
        {
            print_complex_element_json(data, i, j_array);
        }
        j[member_name] = j_array;
    }
    else
        ...
        nlohmann::json& j)
{
    ...
    switch (st_data->type()->get_kind())
    {
        ...
        case TK_ARRAY:
        {
            print_collection_json(st_data, j);
            break;
        }
        case TK_MAP:
        {
