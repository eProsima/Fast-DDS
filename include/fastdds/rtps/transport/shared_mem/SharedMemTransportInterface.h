...
#ifndef SHAREDMEM_TRANSPORT_INTERFACE_H
#define SHAREDMEM_TRANSPORT_INTERFACE_H
...
namespace rtps {
	...
	protected:
		...
		std::vector<SharedMemChannelResource*> mInputSockets;

		uint32_t mSendBufferSize;
		uint32_t mReceiveBufferSize;

		SharedMemTransportInterface(int32_t transport_kind);
		...
		*/
		...
		//virtual eProsimaSharedMem OpenAndBindInputSocket(const std::string& sIp, uint16_t port, bool is_multicast) = 0;

		//eProsimaSharedMem OpenAndBindUnicastOutputSocket(const asio::ip::udp::endpoint& endpoint, uint16_t& port);

		virtual void set_receive_buffer_size(uint32_t size) = 0;
		virtual void set_send_buffer_size(uint32_t size) = 0;
		//virtual void SetSocketOutboundInterface(eProsimaSharedMem&, const std::string&) = 0;
	};
...
#endif // SHAREDMEM_TRANSPORT_INTERFACE_H
