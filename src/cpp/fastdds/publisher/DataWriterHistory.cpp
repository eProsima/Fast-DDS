...
DataWriterHistory::DataWriterHistory(
    ...
    : WriterHistory(HistoryAttributes(mempolicy, payloadMaxSize,
            topic.get_qos().history().kind == KEEP_ALL_HISTORY_QOS ?
            topic.get_qos().resource_limits().allocated_samples :
            !type->m_isGetKeyDefined ?
            std::min(topic.get_qos().resource_limits().allocated_samples, topic.get_qos().history().depth) :
            std::min(topic.get_qos().resource_limits().allocated_samples, topic.get_qos().history().depth
            * topic.get_qos().resource_limits().max_instances),
            topic.get_qos().history().kind == KEEP_ALL_HISTORY_QOS ?
            topic.get_qos().resource_limits().max_samples :
            !type->m_isGetKeyDefined ?
            topic.get_qos().history().depth :
            topic.get_qos().history().depth * topic.get_qos().resource_limits().max_instances))
    , history_qos_(topic.get_qos().history())
    , resource_limited_qos_(topic.get_qos().resource_limits())
    , type_(type)
{
}
        ...
        std::chrono::time_point<std::chrono::steady_clock> max_blocking_time)
{
    if (m_isHistoryFull)
    {
        ...
        if (!ret)
        {
            logWarning(RTPS_HISTORY, "Attempting to add Data to Full WriterCache: " << type_.get_type_name());
            return false;
        }
    }
...
bool DataWriterHistory::find_key(
