        ...
        RequestInfo& info)
{
    ...
    if (!is_fully_matched())
    {
        std::cout << "Trying to take a request with an unmatched requester" << std::endl;
        return RETCODE_PRECONDITION_NOT_MET;
    }
        ...
        const RequestInfo& info) const
{
    ...
    StatusMask new_mask, original_mask = new_mask = reader_condition.get_enabled_statuses();
    new_mask |= StatusMask::subscription_matched();
    reader_condition.set_enabled_statuses(new_mask);
    ...
    reader_condition.set_enabled_statuses(original_mask);
