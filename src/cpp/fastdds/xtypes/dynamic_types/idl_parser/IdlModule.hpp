...
public:
    ...
    bool structure(
            ...
            bool replace = false)
    {
        std::string name(type_builder->get_name());
        ...
        type_builder->get_descriptor(type_descriptor);
        type_descriptor->name(name_space + (name_space.empty() ? "" : "::") + name);
        auto result = structs_.emplace(name, type_builder);
