        ...
        const ParticipantProxyData& participant_data)
            {
                ...
                if (att.getTopicDiscoveryKind() != NO_CHECK)
                {
                    if (att.type_id.m_type_identifier._d() == 0) // Not set
                    {
                        ...
                        {
                            logError(EDP,
                                    "TopicDiscoveryKind isn't NO_CHECK, but type identifier " << rpd->typeName()
                                                                                              << " isn't registered.");
                        }
                        else
                        {
        ...
        const ParticipantProxyData& participant_data)
            {
                ...
                if (att.getTopicDiscoveryKind() != NO_CHECK)
                {
                    if (att.type_id.m_type_identifier._d() == 0) // Not set
                    {
                        ...
                        {
                            logError(EDP,
                                    "TopicDiscoveryKind isn't NO_CHECK, but type identifier " << wpd->typeName()
                                                                                              << " isn't registered.");
                        }
                        else
                        {
            ...
            else
            {
                        ...
                        && (*wit)->matched_reader_remove(rdata->guid()))
                {
                    ...
                    mp_RTPSParticipant->security_manager().remove_reader(
                        (*wit)->getGuid(), participant_guid, rdata->guid());
#endif
                    //MATCHED AND ADDED CORRECTLY:
                    if ((*wit)->getListener() != nullptr)
                    {
            ...
            else
            {
                        ...
                        && (*rit)->matched_writer_remove(wdata->guid()))
                {
                    ...
                    mp_RTPSParticipant->security_manager().remove_writer(
                        (*rit)->getGuid(), participant_guid, wdata->guid());
#endif
                    //MATCHED AND ADDED CORRECTLY:
                    if ((*rit)->getListener() != nullptr)
                    {
