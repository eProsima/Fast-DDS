        ...
        ReaderProxyData* rdata)
{
    ...
    {
        enable_reader_on_v2_reduced_property(localpdata->m_participantName.to_string(), localpdata->m_properties,
                rdata->userDefinedId(), false);
    }
    else
    {
        ...
        WriterProxyData* wdata)
{
    ...
    if (ExchangeFormat::v2_Reduced == exchange_format_)
    {
        enable_writer_on_v2_reduced_property(localpdata->m_participantName.to_string(), localpdata->m_properties,
                wdata->userDefinedId(), false);
    }
    else
    {
        ...
        RTPSReader* rtps_reader)
{
    ...
    if (ExchangeFormat::v2_Reduced == exchange_format_)
    {
        ret_value = enable_reader_on_v2_reduced_property(
            localpdata->m_participantName.to_string(), localpdata->m_properties,
        ...
        bool /*assign_secure_endpoints*/)
{
    ...
    if (ExchangeFormat::v2_Reduced == exchange_format_)
    {
        std::string participant_name {pdata.m_participantName.to_string()};
            ...
            if (0 == pit->first().compare("ESR"))
            {
                ...
                for (size_t i {0}; i < bits.size(); ++i)
                {
                            ...
                            mp_edpXML->get_reader_from_position(participant_name, i, &rdataptr))
                    {
                        ...
                        {
                            newRemoteReader(pdata.m_guid, pdata.m_participantName, rdataptr->userDefinedId());
                        }
                ...
                for (size_t i {0}; i < bits.size(); ++i)
                {
                            ...
                            mp_edpXML->get_writer_from_position(participant_name, i, &wdataptr))
                    {
                        if (bits[i] && !this->mp_PDP->has_writer_proxy_data(wdataptr->guid()))//IF NOT FOUND, we CREATE AND PAIR IT
                        {
                            if (is_persistent_guid)
                            {
                                persistence_guid.entityId.value[3] = static_cast<uint8_t>(wdataptr->userDefinedId());
                            }
                            newRemoteWriter(pdata.m_guid, pdata.m_participantName,
                                    wdataptr->userDefinedId(), EntityId_t::unknown(), persistence_guid);
                        }
        ...
        EntityId_t ent_id)
{
    ...
    if ((0x67 == ent_id.value[3] && // Check the endpoint is a builtin statistic endpoint.
            ...
            xmlparser::XMLP_ret::XML_OK == mp_edpXML->lookforReader(participant_name, user_id, &rpd, position))
    {
        ...
        const GUID_t& persistence_guid)
{
    ...
    if ((0x62 == ent_id.value[3] && // Check the endpoint is a builtin statistic endpoint.
            ...
            xmlparser::XMLP_ret::XML_OK == mp_edpXML->lookforWriter(participant_name, user_id, &wpd, position))
    {
        ...
        const EntityId_t& entity_id)
{
    ...
    if (nullptr != topic_name && nullptr != type_name)
    {
        ...
        writer_data->m_qos.m_reliability.kind = eprosima::fastdds::dds::RELIABLE_RELIABILITY_QOS;
        writer_data->m_qos.m_durability.kind = eprosima::fastdds::dds::TRANSIENT_LOCAL_DURABILITY_QOS;
    }
        ...
        const EntityId_t& entity_id)
{
    ...
    if (nullptr != topic_name && nullptr != type_name)
    {
        ...
        reader_data->m_qos.m_reliability.kind = eprosima::fastdds::dds::RELIABLE_RELIABILITY_QOS;
        reader_data->m_qos.m_durability.kind = eprosima::fastdds::dds::TRANSIENT_LOCAL_DURABILITY_QOS;
    }
