        ...
        ParticipantProxyData* participant_data)
{
    ...
    participant_data->lease_duration = mp_RTPSParticipant->get_attributes().builtin.discovery_config.leaseDuration;
    ...
    if (mp_RTPSParticipant->get_attributes().builtin.use_WriterLivelinessProtocol)
    {
    ...
    {
        participant_data->m_network_configuration = mp_RTPSParticipant->get_attributes().builtin.network_configuration;

        LocatorList_t default_unicast_locators = *mp_RTPSParticipant->get_default_unicast_snapshot();
        for (const Locator_t& loc : default_unicast_locators)
        {
            participant_data->default_locators.add_unicast_locator(loc);
        }
        LocatorList_t default_multicast_locators = mp_RTPSParticipant->get_attributes().defaultMulticastLocatorList;
        for (const Locator_t& loc : default_multicast_locators)
        {
    ...
    // Keep persistence Guid_Prefix_t in a specific property. This info must be propagated to all builtin endpoints
    {
        ...
        {
            persistent = mp_RTPSParticipant->get_attributes().prefix;
        }
    ...
    if (announce_locators)
    {
        ...
        fastdds::rtps::network::external_locators::add_external_locators(*participant_data,
                *mp_RTPSParticipant->get_metatraffic_ext_unicast_snapshot(),
                *mp_RTPSParticipant->get_default_ext_unicast_snapshot());
    }

    participant_data->participant_name = std::string(mp_RTPSParticipant->get_attributes().getName());

    participant_data->user_data = *mp_RTPSParticipant->get_user_data_snapshot();
    ...
    participant_data->wire_protocol->participant_id = mp_RTPSParticipant->get_attributes().participantID;
    BuiltinAttributes builtin_copy;
    {
        std::lock_guard<std::recursive_mutex> guardPDP(*mp_mutex);
        builtin_copy = mp_RTPSParticipant->get_attributes().builtin;
    }
    participant_data->wire_protocol->builtin = builtin_copy;
    participant_data->wire_protocol->port = mp_RTPSParticipant->get_attributes().port;
    participant_data->wire_protocol->default_unicast_locator_list = *mp_RTPSParticipant->get_default_unicast_snapshot();
    participant_data->wire_protocol->default_multicast_locator_list = mp_RTPSParticipant->get_attributes().defaultMulticastLocatorList;
    participant_data->wire_protocol->default_external_unicast_locators = *mp_RTPSParticipant->get_default_ext_unicast_snapshot();
    participant_data->wire_protocol->ignore_non_matching_locators = mp_RTPSParticipant->get_attributes().ignore_non_matching_locators;
        ...
        RTPSParticipantImpl* part)
{
    ...
    initial_announcements_ = mp_RTPSParticipant->get_attributes().builtin.discovery_config.initial_announcements;
    //CREATE ENDPOINTS
        ...
        if (pit->guid.guidPrefix == reader_guid.guidPrefix)
        {
            ...
            if (reader_proxies_pool_.empty())
            {
                ...
                if (reader_proxies_number_ < max_proxies)
                {
                    ...
                    const auto& allocations = mp_RTPSParticipant->get_attributes().allocation;
...
void PDP::set_external_participant_properties_(
        ParticipantProxyData* participant_data)
{
    const RTPSParticipantAttributes& part_attributes = mp_RTPSParticipant->get_attributes();
        ...
        const BuiltinProtocols* builtin)
{
    ...
    endpoint.ignore_non_matching_locators = pdp->getRTPSParticipant()->get_attributes().ignore_non_matching_locators;
}
