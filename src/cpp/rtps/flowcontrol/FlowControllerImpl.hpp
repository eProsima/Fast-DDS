...
struct FlowQueue
{
    ...
    void add_new_sample(
            fastdds::rtps::CacheChange_t* change) noexcept
    {
    ...
    fastdds::rtps::CacheChange_t* get_next_change() noexcept
    {
        if (!is_empty())
        {
...
private:
    ...
    struct ListInfo
    {
                ...
                ListInfo& list) noexcept
        {
            if (!list.is_empty())
            {
                fastdds::rtps::CacheChange_t* first = list.head.writer_info.next;
                fastdds::rtps::CacheChange_t* last = list.tail.writer_info.previous;
        ...
        fastdds::rtps::CacheChange_t head;
        fastdds::rtps::CacheChange_t tail;
    };
...
struct FlowControllerAsyncPublishMode
{
    ...
    bool fast_check_is_there_slot_for_change(
            fastdds::rtps::CacheChange_t*) const
    {
        return true;
    }
    ...
    void process_deliver_retcode(
            const fastdds::rtps::DeliveryRetCode&)
    {
    }
    ...
    fastdds::rtps::RTPSMessageGroup group;
...
struct FlowControllerLimitedAsyncPublishMode : public FlowControllerAsyncPublishMode
{
    ...
    void process_deliver_retcode(
            const fastdds::rtps::DeliveryRetCode& ret_value)
    {
        if (fastdds::rtps::DeliveryRetCode::EXCEEDED_LIMIT == ret_value)
        {
            force_wait_ = true;
        }
    }
...
struct FlowControllerFifoSchedule
{
     ...
     * bandwidth exceeded.
     */
    fastdds::rtps::CacheChange_t* get_next_change_nts()
    {
...
{
    using element = std::tuple<fastdds::rtps::RTPSWriter*, FlowQueue>;
    ...
    {
        fastdds::rtps::RTPSWriter* current_writer = nullptr;
            ...
            fastdds::rtps::RTPSWriter* writer)
    {
        ...
        fastdds::rtps::RTPSWriter* current_writer = std::get<0>(*next_writer_);
    ...
    fastdds::rtps::CacheChange_t* get_next_change_nts()
    {
        fastdds::rtps::CacheChange_t* ret_change = nullptr;
...
private:
    ...
    iterator find(
            const fastdds::rtps::RTPSWriter* writer)
    {
...
struct FlowControllerHighPrioritySchedule
{
            ...
            fastdds::rtps::RTPSWriter* writer)
    {
        ...
        int32_t priority = 10;
        auto property = fastdds::rtps::PropertyPolicyHelper::find_property(
...
private:
    ...
    std::unordered_map<fastdds::rtps::RTPSWriter*, int32_t> priorities_;
};
...
struct FlowControllerPriorityWithReservationSchedule
{
            ...
            fastdds::rtps::RTPSWriter* writer)
    {
        ...
        property = fastdds::rtps::PropertyPolicyHelper::find_property(
    ...
    fastdds::rtps::CacheChange_t* get_next_change_nts()
    {
        fastdds::rtps::CacheChange_t* highest_priority = nullptr;
        fastdds::rtps::CacheChange_t* ret_change = nullptr;
        ...
        if (0 < writers_queue_.size())
        {
            for (auto& priority : priorities_)
            {
                for (auto writer_it : priority.second)
                {
                    ...
                    fastdds::rtps::CacheChange_t* change = std::get<0>(writer->second).get_next_change();
...
private:
    ...
    using map_writers = std::unordered_map<fastdds::rtps::RTPSWriter*, std::tuple<FlowQueue, int32_t, uint32_t,
                    uint32_t>>;

    using map_priorities = std::map<int32_t, std::vector<fastdds::rtps::RTPSWriter*>>;
    ...
    fastdds::rtps::RTPSWriter* writer_being_processed_ = nullptr;
...
public:
     ...
     */
    void register_writer(
            fastdds::rtps::RTPSWriter* writer) override
    {
...
private:
            ...
            const std::chrono::time_point<std::chrono::steady_clock>& max_blocking_time)
    {
        ...
        fastdds::rtps::LocatorSelectorSender& locator_selector = writer->get_general_locator_selector();
#if HAVE_STRICT_REALTIME
        std::unique_lock<fastdds::rtps::LocatorSelectorSender> lock(locator_selector, std::defer_lock);
        if (lock.try_lock_until(max_blocking_time))
#else
        std::unique_lock<fastdds::rtps::LocatorSelectorSender> lock(locator_selector);
#endif // if HAVE_STRICT_REALTIME{
        {
            try
            {
                fastdds::rtps::RTPSMessageGroup group(participant_, writer, &locator_selector, max_blocking_time);
                ret_value = true;
                if (fastdds::rtps::DeliveryRetCode::DELIVERED !=
            ...
            catch (fastdds::rtps::RTPSMessageGroup::timeout&)
            {
            }
        }
    ...
    void run()
    {
        while (async_mode.running)
        {
            ...
            std::unique_lock<fastdds::TimedMutex> lock(mutex_);
            fastdds::rtps::CacheChange_t* change_to_process = nullptr;
            ...
            while (nullptr != change_to_process)
            {
                ...
                fastdds::rtps::LocatorSelectorSender& locator_selector =
                ...
                fastdds::rtps::CacheChange_t* previous = change_to_process->writer_info.previous;
                fastdds::rtps::CacheChange_t* next = change_to_process->writer_info.next;
                ...
                fastdds::rtps::DeliveryRetCode ret_delivery = current_writer->deliver_sample_nts(
                    change_to_process, async_mode.group, locator_selector,
                    std::chrono::steady_clock::now() + std::chrono::hours(24));

                if (fastdds::rtps::DeliveryRetCode::DELIVERED != ret_delivery)
                {
    ...
    std::map<fastdds::rtps::GUID_t, fastdds::rtps::RTPSWriter*> writers_;
