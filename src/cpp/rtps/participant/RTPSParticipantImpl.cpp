...
static std::vector<unsigned char> read_file(
    const std::string& path)
{
    ...
    // got to the beginning and read all the file
    f.seekg(0, std::ios::beg);
    std::vector<unsigned char> ret(static_cast<size_t>(n));
    if (!f.read(reinterpret_cast<char*>(ret.data()), n))
    {
...
// load x509 of the certificate
//  - first it tries to get PEM.
//  - if that fails, try DER.
static X509* load_x509_any(const std::string& cert_path) {
    ...
    // PEM value of the certificate
    X509* cert = PEM_read_bio_X509(bio, nullptr, nullptr, nullptr);
    if (!cert)
    {
        BIO_seek(bio, 0);
        // DER value of the certificate
...
// load public key (Certi_ePro.cer)
static EVP_PKEY* get_pubkey_from_cert(
    const std::string& cert_path)
{
    X509* cert = load_x509_any(cert_path);
    if (!cert)
    {
        throw std::runtime_error("Failed to load X509 certificate: " + cert_path);
    }

    // extract the key
    EVP_PKEY* ret = X509_get_pubkey(cert);
    X509_free(cert);
    if (!ret)
    {
    ...
    const std::string& json_path)
{
    ...
    {
        throw std::runtime_error("JSON does not contain an object field named 'payload'");
    }

    const nlohmann::json& payload = j["payload"];
    std::map<std::string, std::string> ret;  // sorted by key
    ...
    for (auto it = payload.begin(); it != payload.end(); ++it)
    {
        ...
        if (value.is_string())
        {
            ret.emplace(it.key(), value.get<std::string>());
        }
        else
        {
            // canonical minified JSON for non-strings
            ret.emplace(it.key(), value.dump());
        }
    }
...
{
    auto not_before = parse_utc_z(utc_not_before);
    //not_before = parse_utc_z("2025-09-01T00:00:00Z");
...
static int check_license()
{
    ...
    const std::string license_path = (std::string) fastddshome_val + "/include/data.sig";
    const std::string json_path = (std::string) fastddshome_val + "/include/fastdds_pro_evaluation.json";
    const std::string pub_key = (std::string) fastddshome_val + "/src/fastdds/include/Certi_ePro.cer";

    try {
        // load inputs from $FASTDDSHOME/include
        std::vector<unsigned char> json_data = read_file(json_path);
        std::vector<unsigned char> license_data  = read_file(license_path);

        // load public key (Certi_ePro.cer)
        EVP_PKEY* pkey = get_pubkey_from_cert(pub_key);
        ...
        const auto json_map = read_license_payload_vars(json_path);
        ...
        {
            EPROSIMA_LOG_ERROR(RTPS_PARTICIPANT, "Signature INVALID: 'running_time' not found.");
        }
        int running_time = std::stoi(it_running_time->second);
        if(running_time <= 0)
        {
            EPROSIMA_LOG_ERROR(RTPS_PARTICIPANT, "Signature INVALID: 'running_time' <= 0.");
        }
