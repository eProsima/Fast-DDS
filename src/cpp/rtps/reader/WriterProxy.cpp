    ...
    , guid_as_vector_(ResourceLimitedContainerConfig::fixed_size_configuration(1u))
{
    ...
    writer_proxy_liveliness_ =
        new WriterProxyLiveliness(
            reader,
            attributes_.guid,
            TimeConv::Duration_t2MilliSecondsDouble(attributes_.livelinessLeaseDuration));
    heartbeat_response_ =
        new HeartbeatResponseDelay(
            this,
            TimeConv::Duration_t2MilliSecondsDouble(reader_->getTimes().heartbeatResponseDelay));
    initial_acknack_ =
        new InitialAckNack(
            this,
            TimeConv::Duration_t2MilliSecondsDouble(reader_->getTimes().initialAcknackDelay));
