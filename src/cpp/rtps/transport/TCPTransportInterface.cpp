        ...
        const Locator& locator)
{
    ...
    if (logical_port != 0)
    {
        ...
        for (auto& sender_resource : send_resource_list)
        {
            ...
            {
                // TODO Carlos: TCP sender resource no longer has a channel, need to be sure the logical port is added elsewhere
                ...
                // }
                // TODO Carlos: Logical port is added in the send()
        ...
        if (channel_resource != channel_resources_.end())
        {
            ...
            // socketsLock.unlock(); // Linked with large_dataLock of 772
        ...
        else
        {
            ...
            if (config != nullptr)
            {
                auto create_connect_channel = [&]()
                        {
                            ...
                            // socketsLock.lock(); // Linked with large_dataLock of 772
                ...
                {
                    // TODO Carlos: Probably not needed anymore
                    // std::unique_lock<std::mutex> large_dataLock(large_data_mutex_);
        ...
        const Locator& remote_locator)
{
    ...
    if (channel->connection_established())
    {
        ...
        {
            channel->add_logical_port(logical_port, rtcp_message_manager_.get());
            // std::cout << " [SEND] Logical Port " << logical_port << " added to channel " << channel->locator() << std::endl;
        }
    }
        ...
        SendResourceList& send_resource_list) const
{
    ...
    for (auto it = send_resource_list.begin(); it != send_resource_list.end();)
    {
        ...
        auto channel_resource = channel_resources_.find(tcp_sender_resource->locator());

        if (tcp_sender_resource &&
                channel_resource->second->connection_status() ==
