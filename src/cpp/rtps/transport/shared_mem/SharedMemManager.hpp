...
private:
    ...
    static inline uint32_t compute_per_allocation_extra_size(
            const std::string& domain_name)
    {
        return SharedMemSegment::compute_per_allocation_extra_size(std::alignment_of<BufferNode>::value, domain_name);
    }
...
public:
            ...
            const std::string& domain_name)
    {
        ...
        uint32_t extra_size = compute_per_allocation_extra_size(domain_name);
