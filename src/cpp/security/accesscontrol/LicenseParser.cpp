...
void LicenseParser::swap(
        //LicenseAccessInfo& info)
        LicenseInfo& info)
{
    info = std::move(access_info_);
}

bool LicenseParser::parse_stream(
        const char* stream,
        size_t stream_length)
{
    assert(stream);

    // Parse without throwing; accept C/C++ style comments if present.
    nlohmann::json doc = nlohmann::json::parse(stream, stream + stream_length, /*callback*/nullptr,
    ...
    // The JSON can be either { "payload": {...} } or directly the payload object.
    const nlohmann::json* payload = &doc;
    ...
    auto get_str_from_payload = [&](const char* key, std::string& dst) -> bool
    {
        ...
        dst = (*payload)[key].get<std::string>();
        return true;
    };

    std::string running_time;

    if (!get_str_from_payload("product_id", access_info_.product_id) ||
        !get_str_from_payload("license_id", access_info_.license_id) ||
        !get_str_from_payload("license_version", access_info_.license_version) ||
        !get_str_from_payload("holder", access_info_.holder) ||
        !get_str_from_payload("issued_at", access_info_.issued_at) ||
        !get_str_from_payload("not_before", access_info_.not_before) ||
        !get_str_from_payload("expires_at", access_info_.expires_at) ||
        !get_str_from_payload("usage", access_info_.usage) ||
        !get_str_from_payload("running_time", running_time))
    {
        return false;
    }

    access_info_.running_time = stoi(running_time);

    return true;
}
/*
bool LicenseParser::parse_domain_access_rules_node(
...
bool LicenseParser::parse_domain_access_rules(
        tinyxml2::XMLElement* root)
{
    assert(root);
...
bool LicenseParser::parse_domain_rule(
        tinyxml2::XMLElement* root,
        DomainRule& rule)
{
    assert(root);
...
#include <iostream>
bool LicenseParser::parse_topic_access_rules(
        tinyxml2::XMLElement* root,
...
bool LicenseParser::parse_topic_rule(
        tinyxml2::XMLElement* root,
        TopicRule& rule)
{
    assert(root);
