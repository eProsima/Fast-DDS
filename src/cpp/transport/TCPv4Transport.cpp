...
{
    //logInfo(RTCP, " SEND [RTPS]: " << sendBufferSize << " bytes to locator " << remoteLocator.get_logical_port());
...
void TCPv4Transport::SocketAccepted(TCPAcceptor* acceptor, const asio::error_code& error, asio::ip::tcp::socket socket)
{
    ...
    if (mSocketsAcceptors.find(acceptor->mLocator.get_physical_port()) != mSocketsAcceptors.end())
    {
        if (!error.value())
        {
            ...
            logInfo(RTCP, " Accepted connection (physical: " << acceptor->mLocator.get_physical_port() << ")");
        }
        else
        {
...
void TCPv4Transport::SocketConnected(Locator_t& locator, SenderResource *senderResource, const asio::error_code& error)
{
    ...
    if (mPendingOutputSockets.find(locator) != mPendingOutputSockets.end())
    {
        ...
        if (!error.value())
        {
            ...
            logInfo(RTCP, " Socket Connected (physical: " << locator.get_physical_port() << ")");
