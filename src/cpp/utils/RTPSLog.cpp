...
{
	std::stringstream ss;
	ss << B_RED << "[  Error] " << DEF <<  RTPSLog::Error.str() << DEF;
...
{
	std::stringstream ss;
		ss << B_YELLOW << "[Warning] " << DEF <<  RTPSLog::Warning.str() << DEF;
...
{
	std::stringstream ss;
		ss << B_GREEN << "[   Info] " << DEF <<  RTPSLog::Info.str() << DEF;
...
{
	RTPSLog::printString(EPROSIMA_LONGINFO_VERBOSITY_LEVEL,RTPSLog::LongInfo.str());
