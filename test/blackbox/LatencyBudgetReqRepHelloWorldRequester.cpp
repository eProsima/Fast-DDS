...
/**
 * @file LatencyBudgetReqRepHelloWorldRequester.cpp
 *
 */
...
LatencyBudgetReqRepHelloWorldRequester::LatencyBudgetReqRepHelloWorldRequester(): reply_listener_(*this), request_listener_(*this),
...
LatencyBudgetReqRepHelloWorldRequester::~LatencyBudgetReqRepHelloWorldRequester()
{
    if(participant_ != nullptr)
        Domain::removeParticipant(participant_);
}

void LatencyBudgetReqRepHelloWorldRequester::init(eprosima::fastrtps::Duration_t latency_budget_duration_pub, eprosima::fastrtps::Duration_t latency_budget_duration_sub)
{
...
void LatencyBudgetReqRepHelloWorldRequester::newNumber(SampleIdentity related_sample_identity, uint16_t number)
{
...
void LatencyBudgetReqRepHelloWorldRequester::block()
{
    std::unique_lock<std::mutex> lock(mutex_);
...
void LatencyBudgetReqRepHelloWorldRequester::wait_discovery(std::chrono::seconds timeout)
{
...
void LatencyBudgetReqRepHelloWorldRequester::matched()
{
    std::unique_lock<std::mutex> lock(mutexDiscovery_);
    ++matched_;
    if(matched_ > 1)
...
bool LatencyBudgetReqRepHelloWorldRequester::is_matched()
{
    return matched_ > 1;
}

void LatencyBudgetReqRepHelloWorldRequester::ReplyListener::onNewDataMessage(Subscriber *sub)
{
...
void LatencyBudgetReqRepHelloWorldRequester::send(const uint16_t number)
{
    WriteParams wparams;
    HelloWorld hello;
    hello.index(number);
