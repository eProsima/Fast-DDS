...
PubSubKeepAllReader::PubSubKeepAllReader(): listener_(*this), lastvalue_(std::numeric_limits<uint16_t>::max()),
    participant_(nullptr), subscriber_(nullptr), initialized_(false), matched_(0), data_received_(0)
{
}

PubSubKeepAllReader::~PubSubKeepAllReader()
{
...
void PubSubKeepAllReader::init(uint16_t nmsgs)
{
...
void PubSubKeepAllReader::destroy()
{
    if(participant_ != nullptr)
    {
        Domain::removeParticipant(participant_);
...
void PubSubKeepAllReader::newNumber(uint16_t number)
{
...
std::list<uint16_t> PubSubKeepAllReader::getNonReceivedMessages()
{
    std::unique_lock<std::mutex> lock(mutex_);
    return msgs_;
}

void PubSubKeepAllReader::read(uint16_t lastvalue, const std::chrono::seconds &seconds)
{
    std::unique_lock<std::mutex> lock(mutex_);
...
void PubSubKeepAllReader::waitDiscovery()
{
...
void PubSubKeepAllReader::waitRemoval()
{
    std::unique_lock<std::mutex> lock(mutexDiscovery_);
...
void PubSubKeepAllReader::matched()
{
    std::unique_lock<std::mutex> lock(mutexDiscovery_);
    ++matched_;
...
void PubSubKeepAllReader::unmatched()
{
    std::unique_lock<std::mutex> lock(mutexDiscovery_);
    --matched_;
    cvDiscovery_.notify_one();
...
void PubSubKeepAllReader::data_received()
{
    std::unique_lock<std::mutex> lock(mutex_);
    ++data_received_;
    cv_.notify_one();
...
void PubSubKeepAllReader::Listener::onNewDataMessage(Subscriber *sub)
{
