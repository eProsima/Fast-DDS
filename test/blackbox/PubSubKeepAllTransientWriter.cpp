...
/**
 * @file PubSubKeepAllTransientWriter.cpp
 *
 */
...
PubSubKeepAllTransientWriter::PubSubKeepAllTransientWriter(): listener_(*this), participant_(nullptr),
    publisher_(nullptr), initialized_(false), matched_(0)
{
}

PubSubKeepAllTransientWriter::~PubSubKeepAllTransientWriter()
{
    if(participant_ != nullptr)
        Domain::removeParticipant(participant_);
}

void PubSubKeepAllTransientWriter::init()
{
	//Create participant
    ...
    puattr.qos.m_durability.kind = eprosima::fastrtps::TRANSIENT_LOCAL_DURABILITY_QOS;
...
void PubSubKeepAllTransientWriter::destroy()
{
    if(participant_ != nullptr)
    {
        Domain::removeParticipant(participant_);
...
void PubSubKeepAllTransientWriter::matched()
{
...
void PubSubKeepAllTransientWriter::unmatched()
{
    std::unique_lock<std::mutex> lock(mutex_);
    --matched_;
    cv_.notify_one();
...
void PubSubKeepAllTransientWriter::waitDiscovery()
{
    std::unique_lock<std::mutex> lock(mutex_);
...
void PubSubKeepAllTransientWriter::waitRemoval()
{
    std::unique_lock<std::mutex> lock(mutex_);
