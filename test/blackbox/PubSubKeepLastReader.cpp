...
PubSubKeepLastReader::PubSubKeepLastReader(): listener_(*this), lastvalue_(std::numeric_limits<uint16_t>::max()),
    participant_(nullptr), subscriber_(nullptr), initialized_(false), matched_(0), data_received_(0)
{
}

PubSubKeepLastReader::~PubSubKeepLastReader()
{
...
void PubSubKeepLastReader::init(uint16_t nmsgs)
{
...
void PubSubKeepLastReader::destroy()
{
    if(participant_ != nullptr)
    {
        Domain::removeParticipant(participant_);
...
void PubSubKeepLastReader::newNumber(uint16_t number)
{
...
std::list<uint16_t> PubSubKeepLastReader::getNonReceivedMessages()
{
    std::unique_lock<std::mutex> lock(mutex_);
    return msgs_;
}

void PubSubKeepLastReader::read(uint16_t lastvalue, const std::chrono::seconds &seconds)
{
    std::unique_lock<std::mutex> lock(mutex_);
...
void PubSubKeepLastReader::waitDiscovery()
{
...
void PubSubKeepLastReader::waitRemoval()
{
    std::unique_lock<std::mutex> lock(mutexDiscovery_);
...
void PubSubKeepLastReader::matched()
{
    std::unique_lock<std::mutex> lock(mutexDiscovery_);
    ++matched_;
...
void PubSubKeepLastReader::unmatched()
{
    std::unique_lock<std::mutex> lock(mutexDiscovery_);
    --matched_;
    cvDiscovery_.notify_one();
...
void PubSubKeepLastReader::data_received()
{
    std::unique_lock<std::mutex> lock(mutex_);
    ++data_received_;
    cv_.notify_one();
...
void PubSubKeepLastReader::Listener::onNewDataMessage(Subscriber *sub)
{
