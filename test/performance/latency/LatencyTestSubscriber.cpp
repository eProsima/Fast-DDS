    ...
    while (RETCODE_OK == ret_code)
    {
        ...
        if (sub->data_loans_)
        {
            ...
            if (RETCODE_OK != ret_code)
            {
                ...
                {
                    EPROSIMA_LOG_ERROR(LatencyTest, "Problem reading Subscriber echoed test data: " << ret_code);
                }
                return;
            }
        ...
        uint32_t datasize)
{
    ...
    if (dynamic_types_)
    {
        EPROSIMA_LOG_ERROR(LatencyTest, "Using dynamic data types for testing");
