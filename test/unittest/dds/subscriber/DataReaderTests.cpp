...
TEST_F(DataReaderTests, get_unread_count)
{
    ...
    EXPECT_EQ(0u, data_reader_->get_unread_count(true));
    EXPECT_EQ(0u, data_reader_->get_unread_count(false));
    EXPECT_EQ(0u, data_reader_->get_unread_count());
}
...
TEST_F(DataReaderTests, read_conditions_wait_on_SampleStateMask)
{
    ...
    EXPECT_EQ(data_reader.get_unread_count(), 1u);
