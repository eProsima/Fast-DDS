...
TEST_P(DynamicTypesPrimitiveTestsAPIs, primitives_apis_unit_tests)
{
    ...
    DynamicTypeBuilder_ptr custom_builder { factory.create_type_copy(*builder1)};
...
TEST_F(DynamicTypesTests, TypeDescriptors_unit_tests)
{
    ...
    DynamicTypeBuilder_ptr builder { factory.create_type_copy(*primitive)};
    ASSERT_TRUE(builder);
    EXPECT_EQ(builder->get_kind(), TypeKind::TK_INT32);

    // Use TypeDescriptor to capture the state
    TypeDescriptor state;
    ASSERT_EQ(primitive->get_descriptor(state), ReturnCode_t::RETCODE_OK);
    ...
    ASSERT_EQ(builder->get_descriptor(state3), ReturnCode_t::RETCODE_OK);
    EXPECT_EQ(state2.copy_from(state3), ReturnCode_t::RETCODE_OK);
...
TEST_F(DynamicTypesTests, DynamicType_basic_unit_tests)
{
    ...
    MemberDescriptor md;
    {
        ...
        ASSERT_NE(struct_type_builder->get_member(0_id), nullptr);
        ASSERT_NE(ReturnCode_t::RETCODE_OK, struct_type_builder->get_member(0_id)->get_descriptor(md));
    }

    // • checking MemberDescriptor getters
    MemberDescriptor md1{3_id, "int32", factory.get_int32_type()};
    md1.set_index(0); // first addition
    ASSERT_NE(struct_type_builder->get_member(3_id), nullptr);
    ASSERT_EQ(ReturnCode_t::RETCODE_OK, struct_type_builder->get_member(3_id)->get_descriptor(md));
    ...
    EXPECT_EQ(DynamicType_ptr{md.get_type()}, DynamicType_ptr{md1.get_type()});
    ...
    ASSERT_NE(struct_type_builder->get_member(1_id), nullptr);
    ASSERT_EQ(ReturnCode_t::RETCODE_OK, struct_type_builder->get_member(1_id)->get_descriptor(md));
    EXPECT_TRUE(md.is_consistent(struct_type_builder->get_kind()));
    ...
    ASSERT_EQ(ReturnCode_t::RETCODE_OK, struct_type_builder->get_member_by_index(0)->get_descriptor(md));
    EXPECT_EQ(md, md1);

    ASSERT_NE(struct_type_builder->get_member_by_index(1), nullptr);
    ASSERT_EQ(ReturnCode_t::RETCODE_OK, struct_type_builder->get_member_by_index(1)->get_descriptor(md));
    EXPECT_EQ(md, md2);

    // • checking by name retrieval
    ASSERT_NE(struct_type_builder->get_member_by_name("int32"), nullptr);
    ASSERT_EQ(ReturnCode_t::RETCODE_OK, struct_type_builder->get_member_by_name("int32")->get_descriptor(md));
    EXPECT_EQ(md, md1);

    ASSERT_NE(struct_type_builder->get_member_by_name("int64"), nullptr);
    ASSERT_EQ(ReturnCode_t::RETCODE_OK, struct_type_builder->get_member_by_name("int64")->get_descriptor(md));
    EXPECT_EQ(md, md2);

    // • checking map indexes retrieval
    //    + indexing by id
    auto members_by_id = struct_type_builder->get_all_members();
    EXPECT_EQ(members_by_id.size(), 2);

    auto dm3 = members_by_id[3_id];
    ASSERT_NE(dm3, nullptr);
    ASSERT_EQ(ReturnCode_t::RETCODE_OK, dm3->get_descriptor(md));
    EXPECT_EQ(md, md1);

    auto dm1 = members_by_id[1_id];
    ASSERT_NE(dm1, nullptr);
    ASSERT_EQ(ReturnCode_t::RETCODE_OK, dm1->get_descriptor(md));
    EXPECT_EQ(md, md1);
    ...
    ASSERT_EQ(ReturnCode_t::RETCODE_OK, members[id]->get_descriptor(md));
    EXPECT_EQ(md, md1);
    ...
    id = members.next_key();
    ASSERT_NE(id, MemberId{});
    ASSERT_EQ(ReturnCode_t::RETCODE_OK, members[id]->get_descriptor(tmp));
    EXPECT_EQ(tmp, md1);
    ...
    // • checking adding duplicates
    {
        ...
        //    + duplicate id
        md = MemberDescriptor(7_id, "dup_bool", factory.get_bool_type());
        EXPECT_NE(ReturnCode_t::RETCODE_OK, struct_type_builder->add_member(md));
    }
}
...
TEST_F(DynamicTypesTests, DynamicType_int32_unit_tests)
{
    ...
    DynamicData_ptr data {DynamicDataFactory::get_instance().create_data(*created_type)};
    ...
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*created_type)};
    ...
    DynamicData_ptr data3 {DynamicDataFactory::get_instance().create_data(*created_type)};
...
TEST_F(DynamicTypesTests, DynamicType_alias_unit_tests)
{
    ...
    DynamicData_ptr aliasData {DynamicDataFactory::get_instance().create_data(*created_type)};
...
TEST_F(DynamicTypesTests, DynamicType_nested_alias_unit_tests)
{
    ...
    DynamicTypeBuilder_ptr alias_struct {factory.create_struct_type()};
    ...
    DynamicTypeBuilder_ptr alias_type32 {factory.create_alias_type(*int32_type, "int32_alias")};
    EXPECT_EQ(alias_struct.add_member(
            0_id,
            "int32",
            *alias_type32.build()),
            ReturnCode_t::RETCODE_OK);

    DynamicType_ptr int64_type {factory.get_int64_type()};
    DynamicTypeBuilder_ptr alias_type64 {factory.create_alias_type(*int64_type, "int64_alias")};
    EXPECT_EQ(alias_struct.add_member(
            1_id,
            "int64",
            *alias_type64.build()),
    ...
    DynamicTypeBuilder_ptr child_alias_struct { factory.create_child_struct_type(*alias_struct.build())};
    ...
    DynamicTypeBuilder_ptr nested_alias_struct { child_alias_struct };
    ...
    do
    {
        ...
        nested_struct.reset(factory.create_child_struct_type(*aux));
        ASSERT_TRUE(nested_struct);
        EXPECT_EQ(nested_struct.add_member(id, member_name, aux), ReturnCode_t::RETCODE_OK);

        aux = DynamicTypeBuilder_cptr {factory.create_alias_type(*nested_alias_struct.build(), alias_name.c_str())}.build();
        nested_alias_struct.reset(factory.create_child_struct_type(*aux));
    ...
    DynamicData_ptr data {DynamicDataFactory::get_instance().create_data(*nested_type)};
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*nested_type)};
    ASSERT_TRUE(data);
    ASSERT_TRUE(data2);
...
TEST_F(DynamicTypesTests, DynamicType_sequence_unit_tests)
{
    ...
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*seq_type)};
    ASSERT_TRUE(pubsubType.deserialize(&payload, data2.get()));
    ASSERT_TRUE(data2->equals(*data));
    ...
    ASSERT_TRUE(static_payload.length == static_payloadSize);
    DynamicData_ptr data3 {DynamicDataFactory::get_instance().create_data(*seq_type)};
    ASSERT_TRUE(pubsubType.deserialize(&static_payload, data3.get()));
    ASSERT_TRUE(data3->equals(*data));
}
...
TEST_F(DynamicTypesTests, DynamicType_sequence_of_sequences_unit_tests)
{
    ...
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*seq_seq_type)};
    ASSERT_TRUE(pubsubType.deserialize(&payload, data2.get()));
    ASSERT_TRUE(data2->equals(*data));
    ...
    ASSERT_TRUE(seqpb.serialize(&seq, &static_payload));
    ASSERT_TRUE(static_payload.length == static_payloadSize);
    DynamicData_ptr data3 {DynamicDataFactory::get_instance().create_data(*seq_seq_type)};
    ASSERT_TRUE(pubsubType.deserialize(&static_payload, data3.get()));
    ASSERT_TRUE(data3->equals(*data));
...
TEST_F(DynamicTypesTests, DynamicType_array_unit_tests)
{
    ...
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*array_type)};
    ASSERT_TRUE(pubsubType.deserialize(&payload, data2.get()));
    ASSERT_TRUE(data2->equals(*data));
    ...
    ASSERT_TRUE(seqpb.serialize(&seq, &static_payload));
    ASSERT_TRUE(static_payload.length == static_payloadSize);
    DynamicData_ptr data3 {DynamicDataFactory::get_instance().create_data(*array_type)};
    ASSERT_TRUE(pubsubType.deserialize(&static_payload, data3.get()));
    ASSERT_TRUE(data3->equals(*data));
}
...
TEST_F(DynamicTypesTests, DynamicType_array_of_arrays_unit_tests)
{
    ...
    // Loan Complex values.
    DynamicData_ptr temp {data.loan_value(testPos)};
    ASSERT_TRUE(temp != nullptr);
    DynamicData_ptr temp2 {data.loan_value(testPos)};
    ...
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*parent_array_type)};
    ASSERT_TRUE(pubsubType.deserialize(&payload, data2.get()));
    ASSERT_TRUE(data2->equals(*data));
    ...
    ASSERT_TRUE(seqpb.serialize(&seq, &static_payload));
    ASSERT_TRUE(static_payload.length == static_payloadSize);
    DynamicData_ptr data3 {DynamicDataFactory::get_instance().create_data(*parent_array_type)};
    ASSERT_TRUE(pubsubType.deserialize(&static_payload, data3.get()));
    ASSERT_TRUE(data3->equals(*data));
}
...
TEST_F(DynamicTypesTests, DynamicType_map_unit_tests)
{
    ...
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*map_type)};
    ASSERT_TRUE(pubsubType.deserialize(&payload, data2.get()));
    ASSERT_TRUE(data2->equals(*data));
    ...
    ASSERT_TRUE(seqpb.serialize(&seq, &static_payload));
    ASSERT_TRUE(static_payload.length == static_payloadSize);
    DynamicData_ptr data3 {DynamicDataFactory::get_instance().create_data(*map_type)};
    ASSERT_TRUE(pubsubType.deserialize(&static_payload, data3.get()));
    ASSERT_TRUE(data3->equals(*data));
}
...
TEST_F(DynamicTypesTests, DynamicType_map_of_maps_unit_tests)
{
    ...
    DynamicData_ptr data {DynamicDataFactory::get_instance().create_data(*map_map_type)};
    ...
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*map_map_type)};
    ASSERT_TRUE(pubsubType.deserialize(&payload, data2.get()));
    ASSERT_TRUE(data2->equals(*data));
    ...
    ASSERT_TRUE(seqpb.serialize(&seq, &static_payload));
    ASSERT_TRUE(static_payload.length == static_payloadSize);
    DynamicData_ptr data3 {DynamicDataFactory::get_instance().create_data(*map_map_type)};
    ASSERT_TRUE(pubsubType.deserialize(&static_payload, data3.get()));
    ASSERT_TRUE(data3->equals(*data));
}
...
TEST_F(DynamicTypesTests, DynamicType_structure_unit_tests)
{
    ...
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*struct_type)};
    ASSERT_TRUE(pubsubType.deserialize(&payload, data2.get()));
    ...
    ASSERT_TRUE(seqpb.serialize(&seq, &static_payload));
    ASSERT_TRUE(static_payload.length == static_payloadSize);
    DynamicData_ptr data3 {DynamicDataFactory::get_instance().create_data(*struct_type)};
    ASSERT_TRUE(pubsubType.deserialize(&static_payload, data3.get()));
...
TEST_F(DynamicTypesTests, DynamicType_structure_inheritance_unit_tests)
{
    ...
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*child_struct_type)};
    ASSERT_TRUE(pubsubType.deserialize(&payload, data2.get()));
    ASSERT_TRUE(data2->equals(*struct_data));
}
...
TEST_F(DynamicTypesTests, DynamicType_multi_structure_unit_tests)
{
    ...
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*parent_struct_type)};
    ASSERT_TRUE(pubsubType.deserialize(&payload, data2.get()));
    ASSERT_TRUE(data2->equals(*struct_data));
    ...
    ASSERT_TRUE(seqpb.serialize(&seq, &static_payload));
    ASSERT_TRUE(static_payload.length == static_payloadSize);
    DynamicData_ptr data3 {DynamicDataFactory::get_instance().create_data(*parent_struct_type)};
    ASSERT_TRUE(pubsubType.deserialize(&static_payload, data3.get()));
    ASSERT_TRUE(data3->equals(*struct_data));
}
...
TEST_F(DynamicTypesTests, DynamicType_union_unit_tests)
{
    ...
    ASSERT_EQ(union_type_builder.add_member(0_id, "first", member_type, "", std::vector<uint64_t>{ 0 }, true),
            ReturnCode_t::RETCODE_OK);
    ASSERT_EQ(union_type_builder.add_member(1_id, "second", another_member_type, "", std::vector<uint64_t>{ 1 },
            false),
            ReturnCode_t::RETCODE_OK);
    ...
    DynamicData_ptr data2 {DynamicDataFactory::get_instance().create_data(*union_type)};
    ASSERT_TRUE(pubsubType.deserialize(&payload, data2.get()));
    ...
    ASSERT_TRUE(seqpb.serialize(&seq, &static_payload));
    ASSERT_TRUE(static_payload.length == static_payloadSize);
    DynamicData_ptr data3 {DynamicDataFactory::get_instance().create_data(*union_type)};
    ASSERT_TRUE(pubsubType.deserialize(&static_payload, data3.get()));
...
TEST_F(DynamicTypesTests, DynamicType_union_with_unions_unit_tests)
{
    ...
    // Add members to the union.
    ASSERT_TRUE(union_type_builder.add_member(0_id, "first", base_type, "", std::vector<uint64_t>{ 0 },
            true) == ReturnCode_t::RETCODE_OK);
    ...
    DynamicData_ptr child_data {union_data.loan_value(1_id)};
