    ...
    std::call_once(once_flag, []()
            {
                ...
                factory->add_type_object("EmptyStruct", GetEmptyStructIdentifier(true),
                GetEmptyStructObject(true));
                factory->add_type_object("EmptyStruct", GetEmptyStructIdentifier(false),
                GetEmptyStructObject(false));

                factory->add_type_object("EmptyInheritanceStruct", GetEmptyInheritanceStructIdentifier(true),
                GetEmptyInheritanceStructObject(true));
                factory->add_type_object("EmptyInheritanceStruct", GetEmptyInheritanceStructIdentifier(false),
                GetEmptyInheritanceStructObject(false));
...
const TypeObject* GetMinimalShortStructObject()
{
    ...
    type_object->minimal().struct_type().struct_flags().IS_MUTABLE(true);
...
const TypeObject* GetCompleteShortStructObject()
{
    ...
    type_object->complete().struct_type().struct_flags().IS_MUTABLE(true);
...

    {
        ...
        //ann.annotation_typeid(GetmutableIdentifier(true));
        ann.annotation_typeid(*TypeObjectFactory::get_instance()->get_type_identifier_trying_complete("mutable"));
...
const TypeIdentifier* GetEmptyStructIdentifier(bool complete)
{
    const TypeIdentifier * c_identifier = TypeObjectFactory::get_instance()->get_type_identifier("EmptyStruct", complete);
    if (c_identifier != nullptr && (!complete || c_identifier->_d() == EK_COMPLETE))
    {
        return c_identifier;
    }

    GetEmptyStructObject(complete); // Generated inside
    return TypeObjectFactory::get_instance()->get_type_identifier("EmptyStruct", complete);
}

const TypeObject* GetEmptyStructObject(bool complete)
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("EmptyStruct", complete);
    if (c_type_object != nullptr)
    {
        return c_type_object;
    }
    ...
    {
        return GetCompleteEmptyStructObject();
    }
    //else
    return GetMinimalEmptyStructObject();
}

const TypeObject* GetMinimalEmptyStructObject()
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("EmptyStruct", false);
    if (c_type_object != nullptr)
    {
        return c_type_object;
    }
    ...
    TypeObjectFactory::get_instance()->add_type_object("EmptyStruct", &identifier, type_object);
    delete type_object;
    return TypeObjectFactory::get_instance()->get_type_object("EmptyStruct", false);
}

const TypeObject* GetCompleteEmptyStructObject()
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("EmptyStruct", true);
    if (c_type_object != nullptr && c_type_object->_d() == EK_COMPLETE)
    {
        return c_type_object;
    }
    ...
    // Header
    type_object->complete().struct_type().header().detail().type_name("EmptyStruct");
    // TODO inheritance
    ...
    TypeObjectFactory::get_instance()->add_type_object("EmptyStruct", &identifier, type_object);
    delete type_object;
    return TypeObjectFactory::get_instance()->get_type_object("EmptyStruct", true);
}

const TypeIdentifier* GetEmptyInheritanceStructIdentifier(bool complete)
{
    const TypeIdentifier * c_identifier = TypeObjectFactory::get_instance()->get_type_identifier("EmptyInheritanceStruct", complete);
    if (c_identifier != nullptr && (!complete || c_identifier->_d() == EK_COMPLETE))
    {
        return c_identifier;
    }

    GetEmptyInheritanceStructObject(complete); // Generated inside
    return TypeObjectFactory::get_instance()->get_type_identifier("EmptyInheritanceStruct", complete);
}

const TypeObject* GetEmptyInheritanceStructObject(bool complete)
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("EmptyInheritanceStruct", complete);
    if (c_type_object != nullptr)
    {
        return c_type_object;
    }
    ...
    {
        return GetCompleteEmptyInheritanceStructObject();
    }
    //else
    return GetMinimalEmptyInheritanceStructObject();
}

const TypeObject* GetMinimalEmptyInheritanceStructObject()
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("EmptyInheritanceStruct", false);
    if (c_type_object != nullptr)
    {
        return c_type_object;
    }
    ...
    //type_object->minimal().struct_type().header().base_type()._d(EK_MINIMAL);
    //type_object->minimal().struct_type().header().base_type().equivalence_hash()[0..13];
    type_object->minimal().struct_type().header().base_type(*GetEmptyStructIdentifier(false));
    ...
    TypeObjectFactory::get_instance()->add_type_object("EmptyInheritanceStruct", &identifier, type_object);
    delete type_object;
    return TypeObjectFactory::get_instance()->get_type_object("EmptyInheritanceStruct", false);
}

const TypeObject* GetCompleteEmptyInheritanceStructObject()
{
    const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("EmptyInheritanceStruct", true);
    if (c_type_object != nullptr && c_type_object->_d() == EK_COMPLETE)
    {
        return c_type_object;
    }
    ...
    // Header
    type_object->complete().struct_type().header().detail().type_name("EmptyInheritanceStruct");
    // TODO inheritance
    type_object->complete().struct_type().header().base_type(*GetEmptyStructIdentifier(true));
    ...
    TypeObjectFactory::get_instance()->add_type_object("EmptyInheritanceStruct", &identifier, type_object);
    delete type_object;
    return TypeObjectFactory::get_instance()->get_type_object("EmptyInheritanceStruct", true);
}
...
const TypeObject* GetMinimalInheritanceStructObject()
{
    ...
    //type_object->minimal().struct_type().header().base_type()._d(EK_MINIMAL);
    //type_object->minimal().struct_type().header().base_type().equivalence_hash()[0..13];
    type_object->minimal().struct_type().header().base_type(*GetShortStructIdentifier(false));
...
const TypeObject* GetCompleteInheritanceStructObject()
{
    ...
    // TODO inheritance
    type_object->complete().struct_type().header().base_type(*GetShortStructIdentifier(true));
