...
TEST(BuiltinDataSerializationTests, safe_dds_signature_parsing)
{
    ...
    // Safe DDS vendor but no signature property
    {
        ...
        ParticipantProxyData out(RTPSParticipantAllocationAttributes{});
        EXPECT_NO_THROW(EXPECT_FALSE(out.readFromCDRMessage(&msg, true, network, false, false, {1, 0x15})));
    }
    ...
    // Safe DDS vendor with signature property, correct length and valid content
    {
        ...
        ParticipantProxyData out(RTPSParticipantAllocationAttributes{});
        EXPECT_NO_THROW(EXPECT_TRUE(out.readFromCDRMessage(&msg, true, network, false, false, {1, 0x15})));
    }
    ...
    // Other vendor with invalid signature property
    {
        ...
        ParticipantProxyData out(RTPSParticipantAllocationAttributes{});
        EXPECT_NO_THROW(EXPECT_TRUE(out.readFromCDRMessage(&msg, true, network, false, false, {1, 1})));
    }
    ...
    for (const auto& test_file : test_files)
    {
        ...
        switch (selector)
        {
            case 0:
            {
                ...
                EXPECT_NO_THROW(pdata.readFromCDRMessage(&msg, true, network, false, false, c_VendorId_eProsima));
                break;
            }
            case 1:
            {
                ReaderProxyData pdata(max_unicast_locators, max_multicast_locators);
                EXPECT_NO_THROW(pdata.readFromCDRMessage(&msg, network, false, false, c_VendorId_eProsima));
                break;
            }
            default:
            {
