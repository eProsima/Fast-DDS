...
TEST(BuiltinDataSerializationTests, safe_dds_signature_parsing)
{
    // Safe DDS vendor but no signature property
    {
        ...
        ParticipantProxyData out(RTPSParticipantAllocationAttributes{});
        EXPECT_NO_THROW(EXPECT_FALSE(out.readFromCDRMessage(&msg, true, network, false)));
    }
...
TEST(BuiltinDataSerializationTests, msg_datasharing_malformed_num_domains)
{
    ...
    // DATA(w)
    {
        ...
        WriterProxyData out(max_unicast_locators, max_multicast_locators);
        ASSERT_FALSE(out.readFromCDRMessage(&msg, network, true));
...
TEST(BuiltinDataSerializationTests, msg_partition_oob_strlen)
{
    ...
    ReaderProxyData out(max_unicast_locators, max_multicast_locators);
    ASSERT_NO_FATAL_FAILURE(out.readFromCDRMessage(&msg, network, true));
}
...
TEST(BuiltinDataSerializationTests, msg_property_list_oversized_num_properties)
{
    ...
    ParticipantProxyData out(RTPSParticipantAllocationAttributes{});
    ASSERT_NO_FATAL_FAILURE(out.readFromCDRMessage(&msg, true, network, true));
    ...
    for (const auto& test_file : test_files)
    {
        ...
        switch (selector)
        {
            case 0:
            {
                ...
                EXPECT_NO_THROW(pdata.readFromCDRMessage(&msg, true, network, false));
                break;
            }
            case 1:
            {
                ReaderProxyData pdata(max_unicast_locators, max_multicast_locators);
                EXPECT_NO_THROW(pdata.readFromCDRMessage(&msg, network, false));
                break;
            }
            default:
            {
