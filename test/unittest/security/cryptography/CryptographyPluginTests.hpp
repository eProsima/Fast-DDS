...
TEST_F(CryptographyPluginTest, factory_RegisterRemoteParticipant)
{
    ...
    ParticipantCryptoHandle *remote_A =CryptoPlugin->m_cryptokeyfactory->register_matched_remote_participant(*local,*i_handle,*perm_handle,*shared_secret, exception);
    ParticipantCryptoHandle *remote_B =CryptoPlugin->m_cryptokeyfactory->register_matched_remote_participant(*local,*i_handle,*perm_handle,*shared_secret, exception);
...
TEST_F(CryptographyPluginTest, exchange_CDRSerializenDeserialize){
    ...
    ParticipantCryptoHandle *ParticipantA = CryptoPlugin->m_cryptokeyfactory->register_local_participant(*i_handle,*perm_handle,prop_handle,exception);
    ...
    std::vector<uint8_t> serialized = CryptoPlugin->m_cryptokeyexchange->KeyMaterialCDRSerialize(base);
    KeyMaterial_AES_GCM_GMAC result = CryptoPlugin->m_cryptokeyexchange->KeyMaterialCDRDeserialize(&serialized);
    ASSERT_TRUE(
...
TEST_F(CryptographyPluginTest, exchange_ParticipantCryptoTokens)
{
    ...
    ParticipantCryptoHandle *ParticipantA = CryptoPlugin->m_cryptokeyfactory->register_local_participant(*i_handle,*perm_handle,prop_handle,exception);
    ParticipantCryptoHandle *ParticipantB = CryptoPlugin->m_cryptokeyfactory->register_local_participant(*i_handle,*perm_handle,prop_handle,exception);

    ASSERT_TRUE( (ParticipantA != nullptr) & (ParticipantB != nullptr) ); 

    //Registrate a remote for both Participants
    ParticipantCryptoHandle *ParticipantA_remote =CryptoPlugin->m_cryptokeyfactory->register_matched_remote_participant(*ParticipantA,*i_handle,*perm_handle,*shared_secret, exception);
    ParticipantCryptoHandle *ParticipantB_remote =CryptoPlugin->m_cryptokeyfactory->register_matched_remote_participant(*ParticipantB,*i_handle,*perm_handle,*shared_secret, exception);

    //Create CryptoTokens for both Participants
    ParticipantCryptoTokenSeq ParticipantA_CryptoTokens, ParticipantB_CryptoTokens;

    ASSERT_TRUE(
            CryptoPlugin->m_cryptokeyexchange->create_local_participant_crypto_tokens(ParticipantA_CryptoTokens, *ParticipantA, *ParticipantA_remote, exception)
    );
    ASSERT_TRUE(
            CryptoPlugin->m_cryptokeyexchange->create_local_participant_crypto_tokens(ParticipantB_CryptoTokens, *ParticipantB, *ParticipantB_remote, exception)
    );

    //Set ParticipantA token into ParticipantB and viceversa
    ASSERT_TRUE(
            CryptoPlugin->m_cryptokeyexchange->set_remote_participant_crypto_tokens(*ParticipantA,*ParticipantA_remote,ParticipantB_CryptoTokens,exception)
    );
     ASSERT_TRUE(
            CryptoPlugin->m_cryptokeyexchange->set_remote_participant_crypto_tokens(*ParticipantB,*ParticipantB_remote,ParticipantA_CryptoTokens,exception)
    );
    ...
    ASSERT_TRUE(Participant_A->RemoteParticipant2ParticipantKeyMaterial.at(1).master_sender_key == Participant_B->RemoteParticipant2ParticipantKeyMaterial.at(1).master_sender_key);
