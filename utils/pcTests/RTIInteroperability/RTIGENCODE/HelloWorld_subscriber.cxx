/* HelloWorld_subscriber.cxx
   ...
   Example subscription of type SampleType automatically generated by 
...
class SampleTypeListener : public DDSDataReaderListener {
  public:
...
void SampleTypeListener::on_data_available(DDSDataReader* reader)
{
    SampleTypeDataReader *SampleType_reader = NULL;
    SampleTypeSeq data_seq;
    DDS_SampleInfoSeq info_seq;
    DDS_ReturnCode_t retcode;
    int i;

    SampleType_reader = SampleTypeDataReader::narrow(reader);
    if (SampleType_reader == NULL) {
        printf("DataReader narrow error\n");
        return;
    }

    retcode = SampleType_reader->take(
    ...
    for (i = 0; i < data_seq.length(); ++i) {
        if (info_seq[i].valid_data) {
            SampleTypeTypeSupport::print_data(&data_seq[i]);
        }
    }

    retcode = SampleType_reader->return_loan(data_seq, info_seq);
...
extern "C" int subscriber_main(int domainId, int sample_count)
{
    ...
    reader_listener = new SampleTypeListener();
    ...
    for (count=0; (sample_count == 0) || (count < sample_count); ++count) {
        ...
        printf("SampleType subscriber sleeping for %d sec...\n",
               receive_period.sec);
