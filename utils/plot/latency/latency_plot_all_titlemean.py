...
rtidata=np.loadtxt("res_rti_latency_2_1.txt",delimiter=',')
eprodata=np.loadtxt("res_epr_latency_2_3.txt",delimiter=',')
...
for col in range(2,3):
	...
	#LINE PLOT
	plt.figure()
	plt.plot(xpos,eprodata[:,col],'-o',color='b',label='eProsima')
	plt.plot(xpos,rtidata[:,col],'-o',color='green',label='RTI-DDS')
	plt.title(r'$Mean\ Round-Trip\ Time$')
	plt.xlim(0.5,11)
	plt.xlabel(r'$Payload\ size\ (bytes)$',fontsize=15)
	plt.ylabel(r'$Time\ (\mu\ s)$',fontsize=15)
	plt.xticks(xpos2,eprodata[:,0].astype(int))
